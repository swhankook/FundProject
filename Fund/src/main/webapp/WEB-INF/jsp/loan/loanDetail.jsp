...
<div class="forum-view">
	<input id="fm-id" type="hidden" value="13010"> <input
		id="fm-pid" type="hidden" value="628576">
	<div class="container">
		<div class="row">
			<div class="col-md-12 col-sm-12 col-xs-12">
				<ul class="forum-user-info">
					<li class="user-image tpl-forum-myimage"><svg
							xmlns="http://www.w3.org/2000/svg" viewBox="0 0 40 40">
							<pattern id="forum-view-image" patternUnits="userSpaceOnUse"
								width="40" height="40">
								<image x="-10" width="60" height="40"
								xmlns:xlink="http://www.w3.org/1999/xlink"
								xlink:href="//i.addblock.net/member/profile_default.png?_1485763659482" />
							</pattern>
							<polygon fill="url(#forum-view-image)"
					...
					<li class="user-info">
						...
						<div class="tpl-forum-hit hit">HIT:${map.HIT_CNT }</div>
					</li>
					<li class="user-ctrl navbar-right"><img id="forum-ctrl"
						...
						<div class="row">
							<form class="form-horizontal mt-75" method="post" id="writeForm">
								<fieldset class="write-pannel">
									...
									<div class="form-group row">
										<label class="col-xs-4 control-label" for="money">희망
											대출금액</label>
										<div class="col-xs-6 inputGroupContainer">
											<div class="input-group">
												<input name="money" id="money" disabled="disabled"
													class="form-control" type="text" dir='rtl'
													value="${map.MONEY }"> <span
													class="input-group-addon">만원</span>
											</div>
										</div>
									</div>
									<div class="form-group row">
										<label class="col-xs-4 control-label" for="period">희망
											대출기간</label>
										<div class="col-xs-6 selectContainer">
											<div class="input-group">
												<div id="period">${map.PERIOD }개월</div>
											</div>
										</div>
									</div>
									<div class="form-group row">
										<label class="col-xs-4 control-label" for="income">연
											소득</label>
										<div class="col-xs-6 inputGroupContainer">
											<div class="input-group ">
												<input name="income" disabled="disabled" id="income"
													class="form-control" type="text" dir='rtl'
													value="${map.INCOME }">
									...
									<div class="form-group row">
										...
										<div class="col-xs-6 inputGroupContainer">
											<div class="input-group">
												<input name="name" id="name" value="${map.NAME }"
										...
										<div class="col-xs-6 inputGroupContainer">
											<div class="input-group">
												<input name="birthday" id="birthday"
													value="${map.BIRTHDAY }" disabled="disabled"
									...
									<div class="form-group row">
										...
										<div class="col-xs-6 inputGroupContainer">
											<div class="input-group">
												<input name="email" id="email" disabled="disabled"
													value="${map.EMAIL }" class="form-control" type="text">
											</div>
										</div>
									</div>
									<div class="form-group row">
										<label class="col-xs-4 control-label" for="phone">휴대폰
											번호</label>
										<div class="col-xs-6 inputGroupContainer">
											<div class="input-group">
												<input name="phone" id="phone" value="${map.PHONE }"
													disabled="disabled" class="form-control" type="text">
											</div>
										</div>
									</div>
					...
					<tbody>
						<tr>
							<td class="comm-area">
								...
								id="PARENT_IDX" value="${map.IDX}">
								<input type="hidden" name="IDX"
								id="IDX" value="${row.IDX}"> <input type="hidden"
								id="userid" name="USERID" value="${row.CREA_ID }"> <textarea
									class="form-control"
									<c:if test="${user.email ne row.CREA_ID}"> disabled="disabled" </c:if>
									id="CONTENTS" name="CONTENTS" data-autoresize="">${row.CONTENTS }</textarea>
								<c:if test="${user.email eq row.CREA_ID}">
									<button class="cm-update cm-controls pull-right"
									style="display: inline;" onclick="loanDetail.update(this)">수정</button>
									<button class="cm-delete cm-controls pull-right"
									style="display: inline;" onclick="loanDetail.del(this)">삭제</button>
								</c:if>
						</tr>
					</tbody>
				</table>
		...
		<div class="container">
			<div class="tpl-comment-form">
				<table class="comment-textarea">
					<tbody>
						<tr>
							<td class="comm-area"><input type="hidden" name="PARENT_IDX"
								id="PARENT_IDX" value="${map.IDX}"> <input type="hidden"
								id="userid" name="USERID" value="${user.email }"> <textarea
									class="form-control" id="CONTENTS" name="CONTENTS"
									placeholder="Write here..." data-autoresize=""></textarea>
								<div class="btn-mobile comment-submit">
									<button class="btn btn-submit btn-round" id="reply"
										type="button" onclick="loanDetail.subLoan();">댓글</button>
								</div></td>
...
<script type="text/javascript">
	var loanDetail = {
		...
		update:function(object) {
			var idx = $(object).parent().find('#IDX').val()
				, contents = $(object).parent().find('#CONTENTS').val()
				, parentIdx = $(object).parent().find('#PARENT_IDX').val();
			var comSubmit = new ComSubmit();
			comSubmit.addParam("IDX", idx);
			comSubmit.addParam("PARENT_IDX", parentIdx);
